...
def run_demo(nest_model, infrared_path, visible_path, output_path_root, index, f_type):
	...
	for img_fusion in img_fusion_list:
		...
		# save images
		for i in range(4):
			output_path_r_temp = output_path_root + 'fusion_nestfuse' + '_' + str(index) + '_temp0' + str(i) + '_ir' + '.png'
			output_path_v_temp = output_path_root + 'fusion_nestfuse' + '_' + str(index) + '_temp0' + str(i) + '_vi' + '.png'
			utils.save_image_test_temp(temp_r_list, i, output_path_r_temp)
			utils.save_image_test_temp(temp_v_list, i, output_path_v_temp)
			print(output_path_r_temp)
			print(output_path_v_temp)
