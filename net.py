...
class ConvTranspose2d_eval(torch.nn.Module):
    def __init__(self):
        super(ConvTranspose2d_eval, self).__init__()
...
class NestFuse_autoencoder(nn.Module):
    ...
    def encoder(self, input):
        x = self.conv0(input)
        x1 = self.DB1_0(x)
        x2 = self.DB2_0(self.pool1(x1))
        x3 = self.DB3_0(self.pool1(x2))
        x4 = self.DB4_0(self.pool1(x3))

        x4_0 = x4
        x3_0 = self._upsample_add(x4_0, x3)
        x2_0 = self._upsample_add(x3_0, x2)
        x1_0 = self._upsample_add(x2_0, x1)
    ...
    def fusion(self, en1, en2, p_type):
        ...
        f1_0 = fusion_function(en1[0], en2[0], "attention_avg")
