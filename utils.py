...
def save_image_test_temp(a,i, output_path):
        a[i] = a[i].mean(dim=1, keepdim=True)
        a[i] = a[i].float()
        a[i] = a[i].cpu().data[0].numpy()
        a[i] = (a[i] - np.min(a[i])) / (np.max(a[i]) - np.min(a[i]))
        a[i] = a[i] * 255
        a[i] = a[i].transpose(1, 2, 0).astype('uint8')
        if a[i].shape[2] == 1:
            a[i] = a[i].reshape([a[i].shape[0], a[i].shape[1]])
        imsave(output_path, a[i])
